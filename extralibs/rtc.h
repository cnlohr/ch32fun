...
void RTC_init() {
	...
	RTC->CTLRL &= ~RTC_FLAG_RSF;               // clear RSF
	while (!(RTC->CTLRL & RTC_FLAG_RSF));      // wait until clocks are bridged
...
void RTC_setAlarm(u32 increments) {
	// Some of these bits reset after alarm trigger
...
void RTC_setAlarmRelative(u16 seconds_from_now, u16 milliseconds_from_now) {
	...
	Delay_Us(10);
	RTC_setAlarm(current + (seconds_from_now * RTC_INCREMENTS_PER_SECOND) + 
